  ...
  addNodeView() {
    return ({ node, getPos, editor }) => {
      ...
      form.addEventListener('submit', async (event) => {
        ...
        for await (const chunk of jsonAgent(messageText,queryText)) {
          ...
          try{
            ...
            if(count>0){
              editor.commands.deleteNode("annotationCreator")
            }
     ...
     editor.commands.insertContent(annotationNode);
        ...
        //     if (previousNode) {
        //       editor.commands.deleteRange({ from: previousNode.pos, to: previousNode.pos + previousNode.nodeSize });
        //     }

        //     // Insert the new content
        //     previousNode = editor.schema.nodeFromJSON(annotationNode);
        //     editor.commands.insertContent(annotationNode);
