...
const url_conversational_lang_memory = TESTURL + "api/query_agent_conversational_lang_memory";
...
export async function* query_agent(query, token) {
  ...
  try {
    ...
    while (true) {
      ...
      console.log("mr. chunk is here", chunk)
...
export async function* query_agent_lang(query, token) {

  let url = url_conversational_lang_memory;
  try {
    const response = await fetch(url, {
      method: "POST",
      headers: {
