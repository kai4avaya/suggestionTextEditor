import {sendToWorker} from "../workers/initiate_worker";
let currChatId;
...
export function vectorizeText(textName, date, type, text, token) {
    
    sendToWorker('create', {"textName": textName, "type": type, "date": date, "text": `${text}`}, token, false)
  }

  export async function searchVectorizedText(dbName, text, k=5, token) {
    let texts;
  
    // Proceed with search if the database is not empty
    texts = await sendToWorker('search', {"dbName": dbName, "text": text, "k": {"limit": k}}, token, false);
